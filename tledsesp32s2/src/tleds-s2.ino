...
void moveback(){
    ...
    for(int jj=0;jj<pantallasizex;jj++){       
      ...
      while((millis()-ini)<tiempo) { servidorweb();if(siguiente){ return;} delay(1);wdt(); }     
...
int getGifInventory(String basePath) {
  ...
  GifRootFolder = SPIFFS.open(basePath);
  ...
  while( file ) {
    if(!file.isDirectory()) {
        ...
        int pos=gifs[amount].indexOf(".gif");

        Serial.println(gifs[amount]);
  ...
  if(req.indexOf("/ ")!=-1){
     ...
     webconfig();
      ...
      if(nombrearchivo!="") {
          ...
          archivo = SPIFFS.open(dirarchivo,FILE_WRITE);             
...
{ // 0=infinite
  ...
  for(;;){
    ...
    if(dif<frameDelay) { 
        ...
        ini=millis();
        while((millis()-ini)<t){ servidorweb();if(siguiente) { break; }delay(1);wdt();}        
    }
...
void loop(){ 
  ...
  }else if(tipo[numpantalla]==8) { //RUN

      runescena();
    
  }else if(tipo[numpantalla]==9) { //RUN

      runmescena();
    
  }else if(tipo[numpantalla]==10) { //TEST

      test();
    
  }else if(tipo[numpantalla]==11) { //COLOR
